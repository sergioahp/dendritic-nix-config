{ lib, ... }: {
  perSystem = { config, pkgs, ... }:
    let
      ...
      cStr = s: if s == null then "NULL" else ''"${lib.escape [ ''"'' "\\" ] s}"'';
      # nsxiv colors go through X, which has no alpha in color strings
      cColor = v: cStr (if v == null then null else colorLib.toRgbHex v);
      resEntry = resource: value: "{ ${cStr resource}, ${value} }";

      mkColorOption = what: lib.mkOption {
        type = lib.types.nullOr lib.types.str;
        default = null;
        description = ''
          ${what} as a hex string (alpha is dropped); null keeps nsxiv's
          built-in default. Compiled in as the fallback value, so it stays
          overridable at runtime through the matching X resource.
        '';
      };
    in
    {
      options.nsxiv = {
        colors = {
          window-bg = mkColorOption "Window background";
          window-fg = mkColorOption "Window foreground";
          bar-bg = mkColorOption "Statusbar background";
          bar-fg = mkColorOption "Statusbar foreground";
          mark-fg = mkColorOption "Mark indicator color";
        };
        barFont = lib.mkOption {
          type = lib.types.nullOr lib.types.str;
          default = "monospace-8";
          description = "Statusbar font (fontconfig pattern); null keeps nsxiv's default";
        };
      };

      # nsxiv has no config file: its config.h is compiled in, so theming goes
      # through the `conf` override of the nixpkgs derivation. This is the
      # upstream config.def.h with the color/font entries parameterized.
      config.packages.nsxiv = pkgs.nsxiv.override {
        conf = ''
          ...
          static const char *WIN_BG[]   = ${resEntry "Nsxiv.window.background" (cColor cfg.colors.window-bg)};
          static const char *WIN_FG[]   = ${resEntry "Nsxiv.window.foreground" (cColor cfg.colors.window-fg)};
          static const char *MARK_FG[]  = ${resEntry "Nsxiv.mark.foreground" (cColor cfg.colors.mark-fg)};
          #if HAVE_LIBFONTS
          static const char *BAR_BG[]   = ${resEntry "Nsxiv.bar.background" (cColor cfg.colors.bar-bg)};
          static const char *BAR_FG[]   = ${resEntry "Nsxiv.bar.foreground" (cColor cfg.colors.bar-fg)};
          static const char *BAR_FONT[] = ${resEntry "Nsxiv.bar.font" (cStr cfg.barFont)};
